...
void *
Blt_Calloc(unsigned int nElem, size_t elemSize)
{
    void *ptr;
    size_t size;
...
Blt_MallocAbortOnError(size_t size, const char *fileName, int lineNum) 
{
    ...
    if (ptr == NULL) {
#ifdef WINDEBUG
	PurifyPrintf("line %d of %s: can't allocate %d bytes of memory\n", 
		lineNum, fileName, size);
#endif
	fprintf(stderr, "line %d of %s: can't allocate %d bytes of memory\n", 
		lineNum, fileName, size);
...
#ifdef WINDEBUG
	PurifyPrintf(
		"line %d of %s: can't allocate %d item(s) of size %d each\n",
		lineNum, fileName, nElem, elemSize);
#endif
	fprintf(stderr, 
		"line %d of %s: can't allocate %d item(s) of size %d each\n", 
		lineNum, fileName, nElem, elemSize);
...
Blt_StrdupAbortOnError(const char *string, const char *fileName, int lineNum)
{
    ...
    if (ptr == NULL) {
#ifdef WINDEBUG
	PurifyPrintf("line %d of %s: can't allocate string of %d bytes\n",
		lineNum, fileName, size);
#endif
	fprintf(stderr, "line %d of %s: can't allocate string of %d bytes\n",
		lineNum, fileName, size);
	fflush(stderr);
	abort();
    }
