...
MakeScanLines(size_t nLines)
{
    ...
    coords = Blt_MallocAssert(sizeof(ScanLine) * nLines);
    for(i = 0; i < nLines; i++) {
    ...
    Blt_Pixel *colorPtr)
{
	...
	for (dy = 0; dy < (lineWidth - r); dy++) {
	    FillHorizontalLine(picture, left, right, y1 + dy, &fill);
	    FillHorizontalLine(picture, left, right, y2 - dy, &fill);
	}
    }
	...
	if (lineWidth > r) {
	    ...
	    midbottom = yOrigin + height - mid - 1;
	} else {
	    midtop = yOrigin + mid;
	    midbottom = yOrigin + height - mid;
	}
	for (y = midtop; y < midbottom; y++) {
	    FillHorizontalLine(picture, left, midleft, y, &fill);
	    FillHorizontalLine(picture, midright, right, y, &fill); 
	}
    }
	...
	if (lineWidth <= r) {
		...
		FillHorizontalLine(picture, x1 - dx1, x1 - dx2, y1 - dy, &fill);
		FillHorizontalLine(picture, x2 + dx2, x2 + dx1, y1 - dy, &fill);
		FillHorizontalLine(picture, x1 - dx1, x1 - dx2, y2 + dy, &fill);
		FillHorizontalLine(picture, x2 + dx2, x2 + dx1, y2 + dy, &fill);
	    }
	}
	...
	for (/* empty */; dy <= r; dy++) {
	    ...
	    dx = outer[dy].right;
	    FillHorizontalLine(picture, x1 - dx, x2 + dx, y1 - dy, &fill);
	    FillHorizontalLine(picture, x1 - dx, x2 + dx, y2 + dy, &fill);
	}
	Blt_Free(outer);
	Blt_Free(inner);
    }
    ...
    Blt_Pixel *colorPtr)
{
	...
	right = xOrigin + width - 1;
	for (y = y1; y <= y2; y++) {
	    FillHorizontalLine(picture, left, right, y, &fill); 
	}
    }
    if (r > 0) {
	ScanLine *outer;
...
static void
xPictDrawCircle(Blt_Picture src, int x, int y, int r, Blt_Pixel *colorPtr)
{
    Blt_Pixel edge, interior;
    double t;
    int r2;
    ...
    Blt_Pixel *colorPtr)
{
	...
	for (y = y1; y <= y2; y++) {
	    FillHorizontalLine(destPtr, x1, x2, y, &interior);
	}
	x1 = x2 = r;
	...
	FillHorizontalLine(destPtr, x1 - x, x2 + x, y2 + y, &interior);
	PaintPixel(destPtr, x2 + x + 1, y2 + y, &edge);

	PaintPixel(destPtr, x1 - x - 1, y1 - y, &edge);
	FillHorizontalLine(destPtr, x1 - x, x2 + x, y1 - y, &interior);
    ...
    FillVerticalLine(destPtr, destX + r, destY, y1, &interior);
	...
	FillVerticalLine(destPtr, x1 - x, y1 - y, y1, &interior); 
	FillVerticalLine(destPtr, x1 - x, y2, y2 + y, &interior); 
	PaintPixel(destPtr, x1 - x, y2 + y + 1, &edge);

	PaintPixel(destPtr, x2 + x, y1 - y - 1, &edge);
	FillVerticalLine(destPtr, x2 + x, y1 - y, y1, &interior); 
	FillVerticalLine(destPtr, x2 + x, y2, y2 + y, &interior); 
...
static void
TintFill(Pict *destPtr, int x, int y, Blt_Pixel *colorPtr)
{
    if ((x >= 0) && (x < destPtr->width) && (y >= 0) && (y < destPtr->height)) {
	Blt_Pixel *dp;
	...
	if (dp->u32 == 0x0) {
#ifdef notdef
	    fprintf(stderr, "%d,%d=%lx, alpha=%x\n", x, y, dp->u32, 
		    dp->Alpha); 
#endif
	    MixColors(dp, colorPtr);
	    TintFill(destPtr, x, y - 1, colorPtr);
	    TintFill(destPtr, x, y + 1, colorPtr);
	    TintFill(destPtr, x - 1, y, colorPtr);
	    TintFill(destPtr, x + 1, y, colorPtr);
	}
    }
}
    ...
    Blt_Pixel *colorPtr)
{
    ...
    dx1 = coords[0].right;
    dx2 = coords[0].left;
    FillHorizontalLine(destPtr, x + dx2, x + dx1, y, &fill);
    FillHorizontalLine(destPtr, x - dx1, x - dx2, y, &fill);
    dx1 = coords[b].right;
    FillHorizontalLine(destPtr, x - dx1, x + dx1, y - b, &fill);
    FillHorizontalLine(destPtr, x - dx1, x + dx1, y + b, &fill);
    for (dy = 1; dy < b; dy++) {
	dx1 = coords[dy].right;
	dx2 = coords[dy].left;
	FillHorizontalLine(destPtr, x + dx2, x + dx1, y - dy, &fill);
	FillHorizontalLine(destPtr, x + dx2, x + dx1, y + dy, &fill);
	FillHorizontalLine(destPtr, x - dx1, x - dx2, y - dy, &fill);
	FillHorizontalLine(destPtr, x - dx1, x - dx2, y + dy, &fill);
    }
    Blt_Free(coords);
}
    ...
    Blt_Pixel *colorPtr)
{
    ...
    dx1 = outer[0].right;
    dx2 = inner[0].left;
    FillHorizontalLine(picture, x + dx2, x + dx1, y, &fill);
    FillHorizontalLine(picture, x - dx1, x - dx2, y, &fill);
    for (dy = 1; dy < (b - lineWidth); dy++) {
	dx1 = outer[dy].right;
	dx2 = inner[dy].left;
	FillHorizontalLine(picture, x + dx2, x + dx1, y - dy, &fill);
	FillHorizontalLine(picture, x + dx2, x + dx1, y + dy, &fill);
	FillHorizontalLine(picture, x - dx1, x - dx2, y - dy, &fill);
	FillHorizontalLine(picture, x - dx1, x - dx2, y + dy, &fill);
    }
    for (/* empty */; dy <= b; dy++) {
	int dx;
	...
	dx = outer[dy].right;
	FillHorizontalLine(picture, x - dx, x + dx, y + dy, &fill);
	FillHorizontalLine(picture, x - dx, x + dx, y - dy, &fill);
    }
    Blt_Free(outer);
    Blt_Free(inner);
    ...
    Blt_Pixel *colorPtr)
{
    ...
    FillHorizontalLine(picture, x - a, x + a, y, &fill);
    for (dy = 1; dy <= b; dy++) {
	dx = coords[dy].right;
	FillHorizontalLine(picture, x - dx, x + dx, y + dy, &fill);
	FillHorizontalLine(picture, x - dx, x + dx, y - dy, &fill);
    ...
    Blt_Pixel *colorPtr)
{
    ...
    if (lineWidth < 1) {
	PictDrawFilledEllipse(picture, x, y, a, b, colorPtr);
    } else {
	PictDrawThickEllipse(picture, x, y, a, b, lineWidth, colorPtr);
    }
}
    ...
    Blt_Pixel *colorPtr)
{
    ...
    if (lineWidth < 1) {
	PictDrawFilledEllipse(picture, x, y, r, r, colorPtr);
    } else if (lineWidth == 1) {
	PictDrawThinEllipse(picture, x, y, r, r, colorPtr);
    } else {
	PictDrawThickEllipse(picture, x, y, r, r, lineWidth, colorPtr);
    }
}
    ...
    Blt_Pixel *colorPtr)
{
    ...
    if (r < 1) {
	PaintRectangle(picture, x, y, width, height, r, lineWidth, colorPtr);
	return;
    }
    region.x = x;
    region.y = y;
    ...
    region.h = height;
    if (!Blt_AdjustRegionToPicture(picture, &region)) {
	return;			/* Rectangular area is totally clipped. */
    }
    /* 
     * Scale the region forming the bounding box of the ellipse into a new
     * picture. The bounding box is scaled by *nSamples* times.
     */
    big = Blt_ScalePicture(picture, region.x, region.y, region.w, region.h, 
		region.w * nSamples, region.h * nSamples);
    if (big != NULL) {
	Blt_Picture tmp;
	...
	PaintRectangle(big,	/* Contains super-sampled original
				 * background. */
	    (x - region.x) * nSamples,
	    (y - region.y) * nSamples,/* Origin of rectangle. */
            width * nSamples,	/* Scaled width */
	    height * nSamples,	/* Scaled height */
	 ...
	 * filter for smoothing. */
	tmp = Blt_CreatePicture(region.w, region.h);
	Blt_ResamplePicture(tmp, big, bltBoxFilter, bltBoxFilter);
	
	/* Replace the bounding box in the original with the new. */
	Blt_CopyPictureBits(picture, tmp, 0, 0, region.w, 
		region.h, region.x, region.y);
	Blt_FreePicture(big);
	Blt_FreePicture(tmp);
    }
}
...
#else
    PaintRectangle(blur, offset*2, offset*2, w, h, r, lineWidth, &color);
    Blt_BlurPicture(blur, blur, offset);
    Blt_BlendPictures(picture, blur, 0, 0, dw, dh, x, y);
    Blt_FreePicture(blur);
#endif
