		    ...
		    int nIntpPts)
{
    ...
    work = Blt_MallocAssert(nOrigPts * sizeof(double));
		  ...
		  int nIntpPts)
{
    ...
    dx = Blt_MallocAssert(sizeof(double) * nOrigPts);
    ...
    A = Blt_MallocAssert(sizeof(TriDiagonalMatrix) * nOrigPts);
    if (A == NULL) {
	Blt_Free(dx);
	return 0;
    }
			   ...
			   int nIntpPts)
{
     ...
     */
    origPts = Blt_MallocAssert((nPoints + 4) * sizeof(Point2d));
