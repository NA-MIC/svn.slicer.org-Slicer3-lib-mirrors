// $Id: Flu_Tree_Browser.cpp,v 1.2 2006/09/22 21:39:07 jjomier Exp $
...
Flu_Tree_Browser::Node* Flu_Tree_Browser :: Node :: insert_branch( const char* fullpath, int pos )
{
  ...
  if( p.size() && p[p.size()-1] != '/')  p += "/"; // && p[p.size()-1] != '\\' ) p += "/";
...
{
  FluSimpleString p( fullpath );
  if( p.size() && ( p[p.size()-1] == '/' ) ) p[p.size()-1] = '\0'; //|| p[p.size()-1] == '\\' ) ) p[p.size()-1] = '\0';
  return insert( p.c_str(), pos );
}
...
int Flu_Tree_Browser :: handle( int event )
{
  ...
  else if( Fl::event_key() == 65289)
  {
      ...
      Fl::focus(this);
    // rdata.hilighted->do_callback( FLU_TAB_KEY );
      rdata.cbReason = FLU_TAB_KEY;
      do_callback();
    return 1;
  }
      // check for the Home key
...
void Flu_Tree_Browser :: Node :: open( bool b )
{
      ...
      if( !tree->rdata.animating && !tree->rdata.animatedNode )
  {
    ...
    tree->rdata.animationOffset = b ? -totalChildH : -1.0f;
