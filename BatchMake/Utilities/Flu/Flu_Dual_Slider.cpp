// $Id: Flu_Dual_Slider.cpp,v 1.1 2006/09/07 02:18:18 jjomier Exp $
...
int Flu_Dual_Slider :: handle( int event )
{
  switch( event )
    ...
    case FL_DRAG:
      {
	float min = (float)minimum(), max = (float)maximum();
	bool flip = false;
	if( min > max )
	  {
	    min = (float)maximum();
	...
	if( _lGrabbed )
	  {
	    ...
	    lowValue = (float)round(lowValue);
	    if( lowValue >= highValue )
	      lowValue = (float)_overlap ? highValue : (float)clamp( increment( highValue, -1 ) );
	    lowValue = (float)clamp(lowValue);
	    if( flip )
	      range( max, min );
	...
	else if( _hGrabbed )
	  {
	    ...
	    highValue = (float)round(highValue);
	    if( highValue <= lowValue )
	      highValue = (float)_overlap ? lowValue : (float)clamp( increment( lowValue, 1 ) );
	    highValue = (float)clamp(highValue);
	    if( flip )
	      range( max, min );
	...
	case FL_Up:
	  if( _horizontal() )
		...
		{
		  set_value( lowValue );
		  lowValue = (float)clamp( increment( lowValue, inc ) );
		  if( lowValue >= highValue )
		    lowValue = (float)_overlap ? highValue : (float)clamp( increment( highValue, inc ) );
		  handle_drag( lowValue );
		  handle_release();
		}
	      else
		{
		  set_value( highValue );
		  highValue = (float)clamp( increment( highValue, inc ) );
		  if( highValue <= lowValue )
		    highValue = _overlap ? lowValue : (float)clamp( increment( lowValue, -inc ) );
		  handle_drag( highValue );
		  handle_release();
		}
	    }
	...
	case FL_Left:
	  if( _horizontal() )
	      ...
	      if( _lFocus )
		{
		  ...
		  lowValue = (float)clamp( increment( lowValue, inc ) );
		  if( lowValue >= highValue )
		    lowValue = _overlap ? highValue : (float)clamp( increment( highValue, inc ) );
		  handle_drag( lowValue );
		  handle_release();
		}
	      else
