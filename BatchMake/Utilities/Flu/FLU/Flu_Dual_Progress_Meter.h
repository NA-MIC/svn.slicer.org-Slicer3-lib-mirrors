// $Id: Flu_Dual_Progress_Meter.h,v 1.1 2006/09/07 02:18:34 jjomier Exp $
...
class FLU_EXPORT Flu_Dual_Progress_Meter
{
 ...
 public:
  ...
  inline float getCurrentValue() const
    { if( currentSlider ) return (float)currentSlider->value(); else return 0.0f; }

  //! \return the current value of the progress bar, on [0,1]
  inline float getTotalValue() const
    { if( totalSlider ) return (float)totalSlider->value(); else return 0.0f; }
  ...
  { if( window ) { if (window->shown()) return true; } else return false; }
