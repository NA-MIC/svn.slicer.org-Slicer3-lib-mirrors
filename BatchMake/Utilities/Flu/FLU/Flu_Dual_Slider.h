// $Id: Flu_Dual_Slider.h,v 1.1 2006/09/07 02:18:34 jjomier Exp $
...
public:
  ...
  { lowValue = v; _lVal = (float)(lowValue-minimum())/(float)(maximum()-minimum()); Fl_Valuator::value(v); }

  //! Get the low value of the slider
  inline float low_value() const
  { return minimum()>maximum() ? (float)(minimum()+maximum()-highValue) : lowValue; }

  //! Set the high value of the slider
  inline void high_value( float v )
  { highValue = v; _hVal = (float)(highValue-minimum())/(float)(maximum()-minimum()); Fl_Valuator::value(v); }

  //! Get the high value of the slider
  inline float high_value() const
  { return minimum()>maximum() ? (float)(minimum()+maximum()-lowValue) : highValue; }
