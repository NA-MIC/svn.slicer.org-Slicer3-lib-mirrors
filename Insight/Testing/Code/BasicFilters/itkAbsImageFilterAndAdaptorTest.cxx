/*=========================================================================
  ...
  Module:    $RCSfile: itkAbsImageFilterAndAdaptorTest.cxx,v $
  Language:  C++
...
int itkAbsImageFilterAndAdaptorTest(int, char* [] ) 
{
  ...
  {
    std::cout <<  ot.Get() << " = ";
    std::cout <<  fabs( it.Get() )  << std::endl; 
    const InputImageType::PixelType  input  = it.Get();
    const OutputImageType::PixelType output = ot.Get();
    const OutputImageType::PixelType absolute  = fabs(input);
    if( fabs( absolute - output ) > epsilon )
    {
